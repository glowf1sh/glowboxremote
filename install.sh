...
#   Uninstall: curl -fsSL https://raw.githubusercontent.com/glowf1sh/glowboxremote/main/install.sh | sudo bash -s -- --uninstall
#
...
CLI_EXPECTED_SHA256="5d214cd34ac5b6b8ba793667196e5489082445dbcf654d6bba553199dca7adbe"
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    if [ ! -z "$LICENSE_KEY" ]; then
        ...
        ACTIVATE_RESPONSE=$(curl -s -X POST https://license.gl0w.bot/api/box/activate \
          -H "Content-Type: application/json" \
          -H "X-Client-Type: glowfish-license-client" \
          -H "X-Client-Auth: glowfish-client-v1-production-key-2025" \
          -H "X-Client-Version: 2.0.0" \
        ...
        if command -v jq &> /dev/null; then
            STATUS=$(echo "$ACTIVATE_RESPONSE" | jq -r '.status // empty' 2>/dev/null)
            JWT_TOKEN=$(echo "$ACTIVATE_RESPONSE" | jq -r '.jwt_token // empty' 2>/dev/null)

            if [ "$STATUS" = "success" ] && [ ! -z "$JWT_TOKEN" ]; then
                echo -e "  ${GREEN}✓${NC} License activated successfully"
                # Update license.json with response data
                if [ -f "$CONFIG_DIR/license.json" ]; then
                    TIER=$(echo "$ACTIVATE_RESPONSE" | jq -r '.tier // "free"' 2>/dev/null)
                    EXPIRES_AT=$(echo "$ACTIVATE_RESPONSE" | jq -r '.expires_at // empty' 2>/dev/null)
                    jq --arg jwt "$JWT_TOKEN" --arg tier "$TIER" --arg expires "$EXPIRES_AT" \
                       '.status = "active" | .jwt_token = $jwt | .tier = $tier | .expires_at = $expires' \
                       "$CONFIG_DIR/license.json" > "$CONFIG_DIR/license.json.tmp" && \
            ...
            echo -e "  ${RED}✗${NC} jq not installed, cannot validate license response"
            echo -e "  ${YELLOW}→${NC} Install jq and try: glowf1sh-license activate YOUR-KEY"
        fi
    fi
else
