...
class CloudClient:
    ...
    async def handle_system_update(self, ws, update_msg: Dict):
        ...
        try:
            ...
            async def send_status(status, message, details=None):
                status_msg = {
                    "type": "update_status",
            ...
            if success:
                ...
                try:
                    ...
                    self.log(f"📤 Sent version update: v{version}")
                except Exception as e:
                    self.log(f"⚠️ Failed to send version update: {e}")
            else:
                self.log(f"❌ System update failed: v{version}")

        except Exception as e:
            self.log(f"❌ System update error (update_handler unavailable): {e}")
            self.log(f"⚠️ Attempting fallback: Direct command execution")

            # FALLBACK: Execute update without update_handler (for bootstrap scenarios)
            try:
                import subprocess
                import base64
                import os
                ...
                await send_status('in_progress', f'Executing update v{version} (fallback mode)')

                # 1. Execute commands FIRST (e.g., create log directories)
                commands = update_package.get('commands', [])
                if commands:
                    self.log(f"📝 Executing {len(commands)} command(s) in fallback mode")
                    for cmd in commands:
                        self.log(f"  Running: {cmd}")
                        result = subprocess.run(cmd, shell=True, capture_output=True, text=True, timeout=300)
                        if result.returncode != 0:
                            self.log(f"  ⚠️ Command failed (exit {result.returncode}): {result.stderr}")
                        else:
                            self.log(f"  ✓ Command completed")

                # 2. Write files
                files = update_package.get('files', [])
                if files:
                    self.log(f"📁 Writing {len(files)} file(s) in fallback mode")
                    for file_info in files:
                        path = file_info.get('path')
                        content_b64 = file_info.get('content_base64')
                        mode = file_info.get('mode', '0644')
                ...
                # 3. Restart services
                services = update_package.get('restart_services', [])
                if services:
                    self.log(f"🔄 Restarting {len(services)} service(s) in fallback mode")
                    for service in services:
                        self.log(f"  Restarting: {service}")
                        subprocess.run(['systemctl', 'restart', service], timeout=30)

                self.log(f"✅ Fallback update completed: v{version}")
                await send_status('completed', f'Update v{version} completed (fallback mode)')
                ...
                self.log(f"❌ Fallback update failed: {fallback_error}")
                await ws.send(json.dumps({
                    "type": "update_status",
                    "status": "failed",
                    "message": f"Update failed (fallback also failed): {str(fallback_error)}",
                    "version": version
                }))
